...
class Skip(commands.Cog):
    ...
    async def helper(self, interaction: discord.Interaction):
        ...
        if voice is None or not await able_to_use_commands(interaction, self.bot.variables_for_guilds[interaction.guild_id].is_playing, self.bot.variables_for_guilds[interaction.guild_id].music_channel_id, self.bot.variables_for_guilds[interaction.guild_id].music_role_id, check_role=True):
            return
        ...
        if voice.is_playing():
            ...
            if len(self.bot.variables_for_guilds[interaction.guild_id].user_who_want_to_skip) >= threshold: #enough people
                ...
                await interaction.response.send_message(f'**Skipping? ({len(self.bot.variables_for_guilds[interaction.guild_id].user_who_want_to_skip)}/{threshold} votes needed) or use `force-skip`**')
