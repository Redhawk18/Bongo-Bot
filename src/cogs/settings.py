...
class Settings(commands.GroupCog, group_name="settings"):
    ...
    async def settings_list(self, interaction: discord.Interaction):
        ...
        if music_channel_id is not None:
            embed.add_field(
                name="Music Channel",
                value=self.bot.get_channel(music_channel_id).mention,
            )
        ...
        if music_role_id is not None:
            embed.add_field(
                name="Music Role",
                value=self.bot.get_guild(interaction.guild_id)
                .get_role(music_role_id)
                .mention,
            )
        ...
        embed.add_field(
            name="Volume 🔊", value=self.bot.cache[interaction.guild_id].volume
        )
    ...
    @app_commands.command(
        ...
        description="Sets the current text channel as the only one music commands can be run from",
    )
    ...
    async def settings_set_music_channel(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Set music channel to {interaction.channel.mention}"
        )
        await self.update_database_value(
            "music_channel_id", interaction.channel_id, interaction.guild_id
        )

    @app_commands.command(
        name="reset-music-channel",
        description="Resets the music channel, so any channel can use music commands",
    )
    @app_commands.checks.cooldown(1, 10, key=lambda i: (i.guild_id, i.user.id))
    ...
    @app_commands.command(
        name="set-music-role",
        description="Sets a role users are required to have to use music commands",
    )
    @app_commands.describe(role="The only role that can use music commands")
    @app_commands.checks.cooldown(1, 10, key=lambda i: (i.guild_id, i.user.id))
    async def settings_set_music_role(
        self, interaction: discord.Interaction, role: discord.Role
    ):
        if role.id == interaction.guild_id:  # role is @everyone
            await interaction.response.send_message(
                f"Role can not be set to {self.bot.get_guild(interaction.guild_id).get_role(role.id).mention}"
            )
            return
        ...
        await interaction.response.send_message(
            f"Set music role to {self.bot.get_guild(interaction.guild_id).get_role(role.id).mention}"
        )
        await self.update_database_value("music_role_id", role.id, interaction.guild_id)

    @app_commands.command(
        name="reset-music-role",
        description="Reset the role required to play music commands",
    )
    @app_commands.checks.cooldown(1, 10, key=lambda i: (i.guild_id, i.user.id))
